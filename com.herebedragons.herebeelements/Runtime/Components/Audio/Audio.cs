...
namespace HereBeElements.Audio
{
   /// <summary>
    /// The audio object
    /// </summary>
   public class Audio<T> where T : Enum
   {
        /// <summary>
        /// The ID of the Audio
        /// </summary>
        ...
        public Transform SourceTransform
        {
            ...
            set
            {
                if(value == null)
                {
                    sourceTransform = SoundManager<T>.Gameobject.transform;
                }
                else
                {
        ...
        public Audio(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != SoundManager<T>.Gameobject.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        public void Play(float volume)
        {
            ...
            if (Pooled)
            {
                ...
                bool restoredFromPool = SoundManager<T>.RestoreAudioFromPool(Type, AudioID);
                if (!restoredFromPool)
                {
                    return;
                }
        ...
        public void Update()
        {
            ...
            switch (Type)
            {
                case AudioType.Music:
                    {
                        AudioSource.volume = Volume * SoundManager<T>.GlobalMusicVolume * SoundManager<T>.GlobalVolume;
                        break;
                    }
                case AudioType.Sound:
                    {
                        AudioSource.volume = Volume * SoundManager<T>.GlobalSoundsVolume * SoundManager<T>.GlobalVolume;
                        break;
                    }
                case AudioType.UISound:
                    {
                        AudioSource.volume = Volume * SoundManager<T>.GlobalUISoundsVolume * SoundManager<T>.GlobalVolume;
                        break;
                    }
            }
