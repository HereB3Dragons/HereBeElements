...
{
    [Serializable]
    public class SoundManager<T>: MonoBehaviour where T: Enum
    {

        private Dictionary<T, AudioSource<T>[]> _audioEvents;
        
        public void Play(T action)
        {
            if (_audioEvents == null || !_audioEvents.ContainsKey(action))
                return; 
            AudioSource<T>[] soundsToPlay = _audioEvents[action];
            Play(soundsToPlay); 
        }

        private static void Play(AudioSource<T>[] sounds)
        {
            AudioSource<T> sound = sounds[Random.Range(0, sounds.Length - 1)];
            
            switch (sound.audioType)
            {
                case AudioType.Music:
                    PlayMusic(sound.audioClip);
                    break;
                case AudioType.Sound:
                    PlaySound(sound.audioClip);
                    break;
                case AudioType.UISound:
                    PlayUISound(sound.audioClip);
                    break;
            }
        }
        ...
        {
            _audioEvents = GetComponent<AudioLibrary<T>>().GetSources();
        }
        ...
        protected static SoundManager<T> instance = null;

        private static Dictionary<int, Audio<T>> musicAudio;
        private static Dictionary<int, Audio<T>> soundsAudio;
        private static Dictionary<int, Audio<T>> UISoundsAudio;
        private static Dictionary<int, Audio<T>> audioPool;
        ...
        private void Init()
        {
            if (!initialized)
            {
                musicAudio = new Dictionary<int, Audio<T>>();
                soundsAudio = new Dictionary<int, Audio<T>>();
                UISoundsAudio = new Dictionary<int, Audio<T>>();
                audioPool = new Dictionary<int, Audio<T>>();
        ...
        private static Dictionary<int, Audio<T>> GetAudioTypeDictionary(AudioType audioType)
        {
            Dictionary<int, Audio<T>> audioDict = new Dictionary<int, Audio<T>>();
            switch (audioType)
            {
                case AudioType.Music:
        ...
        private static void UpdateAllAudio(Dictionary<int, Audio<T>> audioDict)
        {
            // Go through all audios and update them
            List<int> keys = new List<int>(audioDict.Keys);
            foreach (int key in keys)
            {
                Audio<T> audio = audioDict[key];
                audio.Update();
        ...
        private static void RemoveNonPersistAudio(Dictionary<int, Audio<T>> audioDict)
        {
        ...
        public static bool RestoreAudioFromPool(AudioType audioType, int audioID)
        {
            ...
            {
                Dictionary<int, Audio<T>> audioDict = GetAudioTypeDictionary(audioType);
        ...
        public static Audio<T> GetAudio(int audioID)
        {
            Audio<T> audio;
        ...
        public static Audio<T> GetAudio(AudioClip audioClip)
        {
            Audio<T> audio = GetMusicAudio(audioClip);
            if (audio != null)
            {
                return audio;
            }
        ...
        public static Audio<T> GetMusicAudio(int audioID)
        {
        ...
        public static Audio<T> GetMusicAudio(AudioClip audioClip)
        {
        ...
        public static Audio<T> GetSoundAudio(int audioID)
        {
        ...
        public static Audio<T> GetSoundAudio(AudioClip audioClip)
        {
        ...
        public static Audio<T> GetUISoundAudio(int audioID)
        {
        ...
        public static Audio<T> GetUISoundAudio(AudioClip audioClip)
        {
            return GetAudio(AudioType.UISound, true, audioClip);
        }

        private static Audio<T> GetAudio(Audio.AudioType audioType, bool usePool, int audioID)
        {
            Dictionary<int, Audio<T>> audioDict = GetAudioTypeDictionary(audioType);
        ...
        private static Audio<T> GetAudio(AudioType audioType, bool usePool, AudioClip audioClip)
        {
            Dictionary<int, Audio<T>> audioDict = GetAudioTypeDictionary(audioType);
        ...
        private static int PrepareAudio(Audio.AudioType audioType, AudioClip clip, float volume, bool loop, bool persist, float fadeInSeconds, float fadeOutSeconds, float currentMusicfadeOutSeconds, Transform sourceTransform)
        {
            if (clip == null)
            {
                Debug.LogError("[ Sound Manager] Audio clip is null", clip);
            }
            ...
            {
                Audio<T> duplicateAudio = GetAudio(audioType, true, clip);
            ...
            Audio<T> audio = new Audio<T>(audioType, clip, loop, persist, volume, fadeInSeconds, fadeOutSeconds, sourceTransform);
